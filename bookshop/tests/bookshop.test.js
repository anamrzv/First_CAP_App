...
describe('Bookshop: OData Protocol Level Testing', () => {
  ...
  const app = require('express')() // http for node.js
  const request = require('supertest')(app) //test library

  beforeAll(async () => {
    await cds.deploy(__dirname + '/../srv/cat-service').to('sqlite::memory:') //wait til cat-serbice is deployed
    await cds.serve('CatalogService').from(__dirname + '/../srv/cat-service').in(app) //connect to http server with "in"
  })
